...
router.post('/', authenticateToken, authorizeRole(1), (req, res) => {
  ...
  roles.push(newRole);
  fs.writeFileSync(path.join(__dirname, '../database.json'), JSON.stringify({ roles, users: JSON.parse(fs.readFileSync(path.join(__dirname, '../database.json'), 'utf8')).users }, null, 2));
